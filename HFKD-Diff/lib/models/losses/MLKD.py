...
class MLKD(nn.Module):
    def __init__(self, ce_weight, kd_weight, tau):
    ...
    def forward(self, logits_s, logits_t, target, **kwargs):
        if logits_s.dim() == 4:
            b, c, _,_ = logits_s.shape
            logits_s = logits_s.view(b, c)
        if logits_t.dim() == 4:
            b, c, _,_ = logits_t.shape
            logits_t = logits_t.view(b, c)
        
        logits_student_weak = logits_s
        logits_student_strong = logits_s
        logits_teacher_weak = logits_t
        logits_teacher_strong = logits_t
